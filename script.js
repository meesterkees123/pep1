...
const allowedTimes = [3, 4, 6, 12];
let maxTimePerQuestion = 6;
let pointsPerQuestion = Math.round(120 / maxTimePerQuestion);
...
function updateTimer() {
    const percentage = (timeLeft / maxTimePerQuestion) * 100;
...
// Start game
function startGame() {
    maxTimePerQuestion = getSelectedTime();
    pointsPerQuestion = Math.round(120 / maxTimePerQuestion);
    score = 0;
    correctAnswers = 0;
    totalQuestions = 0;
    questionCount = 0;
...
function getSelectedTime() {
    const idx = Math.min(
        ...
        Math.max(0, Math.round(Number(timeSlider.value)))
    );
    const val = allowedTimes[idx];
    timeSlider.value = idx;
    timeDisplay.textContent = `${val}s`;
    return val;
}

// Keep slider in sync with allowed values
timeSlider.addEventListener('input', getSelectedTime);
// Initialize display
getSelectedTime();
